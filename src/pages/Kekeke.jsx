    ...
    <PjExplainField>
      <ExBox>
        ...
        <Recall>
          ...
          하지만 프로젝트도중 허리를 다치고, 저희 팀 프로젝트와 일정까지 겹쳐 채팅 기능 구현에는 참여를 하지 못했습니다. <br />
          <br />
          로그인 기능을 구현하는 도중 구매자가 회원가입 할때 제한사항으로 인해 가입이 안되었는데 첨부한 프로필 이미지는 저장되며 <br />
          페이지가 갱신되는 문제를 발견했습니다. <br />
          이 문제를 해결하기 위해 하루종일 고생했었는데, 일단 이미지가 저장되는 원인을 찾아보니 multer를 사용해 이미지를 저장하는데 <br />
          filefilter를 설정하지 않았기 때문이었습니다. <br />
          일반적으로 인터넷에서 알려주는 multer의 기능에는 filter의 내용들이 없었는데, 대부분의 경우 프론트에서 제한사항 체크를 하고 <br />
          서버로 넘겨주기 때문이라고 생각했고, 이 프로젝트의 경우 서버에서 모든 제한사항을 체크하면서 생긴 문제로 보였습니다. <br />
          프로젝트는
