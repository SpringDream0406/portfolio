      ...
      <ExBox>
        ...
        <ProjectBox>
          ...
          <span className="pjContent">
            React / Flask / Oracle / Github / Notion
          </span>
        </ProjectBox>
        <hr />
        ...
        <ProjectBox>
          ...
          <div className="pjContent">
            <a
              className="pjContent"
              href="https://hellosori.notion.site/4dd1ce5f2f684bf9adf2cb49d631c81b"
              target="_blank"
        ...
        <Recall>
          <h4>회고</h4>
          첫 프로젝트였던 만큼 많은 문제와 실제 개발을 경험해 볼 수 있는
          프로젝트였습니다. <br />
          <br />
          가장 기억에 남는 첫 번째는 서버와 DB를 배우지 않은 상태로 프로젝트를
          하게 되었고, 제가 그 서버와 DB를 맡았던 점입니다. <br />
          이 문제를 해결하기 위해 우선 선생님들께 서버와 DB의 이론적인 내용 등을
          여쭤 이론적인 지식을 쌓고, <br />
          저희 반 담당 선생님께 계속 요청하여 2시간 동안 Flask와 Oracle의
          초기설정과 간단한 동작 코드를 배웠습니다. <br />
          이후 블로그와 GPT를 참고하여 React와 Flask의 간단한 동작 예제 코드를
          사용해 보며 기본적인 코드와 동작을 알게 되었고, <br />
          이를 활용하여 Flask로 API 서버를 구축했으며, 교육 전 프리트레이닝을
          통해 배운 DB 내용을 활용하여 Oracle로 DB를 구축하고 관리했습니다.
          <br />
          이때 맨땅에 헤딩하는 느낌의 어려움을 경험했지만, 원하는 기능을
          구축해내면서 뿌듯함을 많이 느낄 수 있었습니다.
          <br />
          <br />
          두 번째는 프로젝트 중 코로나에 걸렸던 것입니다. <br />
          코로나에 걸려서 일주일간 집에 격리되었고, 몸도 많이 안 좋았었는데,
          프로젝트에서 중요한 역할인 서버와 DB를 담당하고 있었습니다. <br />
          게다가 프론트와 API를 연동하는 단계였기 때문에, 학원에 노트북을 서버로
          켜두고, 집에서 원격으로 관리하며, <br />
          요청에 맞춰 API를 만들어주고, 연동을 확인하는 등의 일을 하면서, 맡은
          역할의 책임감을 느끼고, 이를 수행하기 위해 노력했습니다. <br />
          덕분에 같은 팀원분들에게 좋은 인상을 줄 수 있었고, 당시에는 몸이 아파
          힘들었지만 역할을 제대로 수행하고 마무리할 수 있어 뿌듯함을 느낄 수
          있었습니다. <br />
          <br />
          <br />
        </Recall>
